...
int Unit::GetMineralCost() const {
    return Type().MineralCost();
}

int Unit::GetVespeneCost() const {
    return Type().GasCost();
}

int Unit::GetTimeCost() const {
...
{
    return cm::get_unit_tbl()->GetTblString(unit_id + 1);
}
