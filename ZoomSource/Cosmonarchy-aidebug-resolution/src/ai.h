            ...
            class Iterator
            {
                public:
                    Iterator(int pl, int reg) : player(pl), region(reg) {
                        while (player < Limits::ActivePlayers && !player_is_computer(player))
                            player++;
                    }
...
#pragma pack(1)
    ...
    struct PlayerData
    {
        ...
        uint8_t unit_build_limits[3000]; // 0x3f0 // Used to be [0xe4] adjusted to [3000], which is the new gptp limit
