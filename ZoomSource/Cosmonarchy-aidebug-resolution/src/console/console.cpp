...
bool Console::KeyHook(int key, int scan)
{
	...
	switch (scan)
	{
		...
		case ScanRight:
			...
			break;
        default:
            char console_key = cm::should_use_f12_for_console() ? ScanF12 : ScanTilde;
            if (scan == ScanTilde) {
                ignore_next = true;
            }
		   	if (console_key == scan) {
            ...
            if (state == shown) {
                if (scan == ScanTilde || scan == ScanEscape) {
					Hide();
					ResetHistoryNavigation();
                    capture_next = true;
                    return true;
